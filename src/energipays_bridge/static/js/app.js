...
document.addEventListener('alpine:init', () => {
  ...
  // Metrics tabs config store (order + default)
  const _METRICS_TABS = [
    { id: 'grid',     label: 'Grid' },
    { id: 'solar',    label: 'Solar' },
    { id: 'heater',   label: 'Heater' },
    { id: 'diverter', label: 'Diverter' },
    { id: 'battery',  label: 'Battery' },
  ]
  const _loadMC = () => {
    try {
      const s = JSON.parse(localStorage.getItem('metricsTabConfig') || 'null')
      if (s?.order && s?.defaultTab) return s
    } catch {}
    return { order: _METRICS_TABS.map(t => t.id), defaultTab: 'grid' }
  }
  Alpine.store('metrics', {
    _cfg: _loadMC(),
    get metricsTabOrder() {
      return this._cfg.order.map(id => _METRICS_TABS.find(t => t.id === id)).filter(Boolean)
    },
    get defaultTab() { return this._cfg.defaultTab },
    _save() { localStorage.setItem('metricsTabConfig', JSON.stringify(this._cfg)) },
    moveTab(id, dir) {
      const order = this._cfg.order
      const idx = order.indexOf(id)
      if (idx === -1) return
      const newIdx = idx + dir
      if (newIdx < 0 || newIdx >= order.length) return
      order.splice(newIdx, 0, order.splice(idx, 1)[0])
      this._cfg = { ...this._cfg, order: [...order] }
      this._save()
    },
    setDefault(id) {
      this._cfg = { ...this._cfg, defaultTab: id }
      this._save()
    },
  })
  ...
  Alpine.store('app', {
    ...
    _allTabs: [
      ...
      { id: 'mqtt',     label: 'MQTT',     icon: '<img src="static/icons/mqtt-logo.svg" class="w-5 h-5" alt="MQTT"/>' },
    ...
    get mainTabs() { return this._allTabs.filter(t => ['dashboard','analytics','rules','logs'].includes(t.id)) },
    // Mobile "More" sheet: secondary tabs (MQTT only if enabled)
    get moreTabs() { return this._allTabs.filter(t => ['raw','settings'].includes(t.id) || (t.id === 'mqtt' && this.mqttEnabled)) },
